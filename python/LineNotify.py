...
class SendInfoToLineGroup():
    ...
    def __AccessToken(self):
        if self.__state['state'] != 0:
            return
        if self.__envName in self.__conf.keys():
            if "token" in self.__conf[self.__envName]:
                self.__token = self.__conf[self.__envName]["token"]
                self.__setStatus(0, "Load the toekn successfully.")
            else:
                self.__setStatus(-1, "Can not find token.")
        else:
            self.__setStatus(-1, "Can not load the environment.")
