 ...
 */
class RangeOfIPv4Addr {

	// assignment
	private IPv4Addr;
	private mask;
	private subsetEnabled;
	...
	// returned messages
	private setRetMsg(getStatus, getInfo, getData) {
	...
	private fixed8bitLength(get2BitStr) {
	...
	private IPv4Num() {
		...
		if(this.subsetEnabled) {
			this.IPv4Len = 2 ** totalIPv4AddrBitNum - 2;
		} else {
			this.IPv4Len = 2 ** totalIPv4AddrBitNum;
		}
	}
	...
	private concatEachIPSection(p1, p2, p3, p4) {
		return p1 + "." + p2 + "." + p3 + "." + p4;
	}
	
	// find all IPv4 addresses
	private listAllIPAddr() {
		var allIPList = [];
